...
const defaultActivities: Activity[] = [
  { id: 1, title: 'Kaiseki Dinner', time: 'Tonight, 7:00 PM', location: 'Yuki Restaurant', icon: Wine, status: 'confirmed' },
  { id: 2, title: 'Private Onsen', time: 'Tomorrow, 6:00 AM', location: 'Rooftop Bath', icon: Waves, status: 'confirmed' },
  { id: 3, title: 'Ski Lesson', time: 'Dec 21, 9:00 AM', location: 'Grand Hirafu', icon: Snowflake, status: 'pending' },
];
...
export default function GuestPortalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [schedule, setSchedule] = useState<Activity[]>(defaultActivities);
  ...
  useEffect(() => {
    async function fetchGuestData() {
      try {
        // Fetch guest by ID (slug)
        const guestRes = await fetch(`/api/guests?id=${slug}`);
        if (!guestRes.ok) {
        ...
        // Fetch the guest's property
        if (guestData.propertyId) {
          const propRes = await fetch(`/api/properties?id=${guestData.propertyId}`);
          if (propRes.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-stone-900 flex items-center justify-center">
        <div className="text-white/50 text-sm">Loading guest profile...</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-stone-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/70 text-lg mb-2">{error || 'Guest not found'}</p>
          <p className="text-white/40 text-sm mb-4">The guest profile for "{slug}" could not be found.</p>
  ...
  const roomName = property?.name || 'Your Suite';
  const roomFloor = property?.floor || '';
  const roomFeatures = property?.features || [];
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className={`relative z-10 h-screen flex flex-col transition-opacity duration-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <nav className="flex items-center justify-between px-8 py-4 flex-shrink-0">
          ...
          {/* Right - VIP Badge + Logout */}
          <div className="flex items-center gap-3">
            {guest.status === 'vip' && (
              <span className="text-xs px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full border border-amber-500/30 uppercase tracking-wider">
                VIP Guest
              </span>
            )}
            <button
        ...
        <div className="flex-1 flex gap-6 px-8 pb-6 min-h-0">
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-2xl flex-1 flex flex-col min-h-0">
              ...
              <div className="mb-6 flex-shrink-0">
                ...
                <p className="text-base text-white/50 mt-2">{roomName} • {guest.nights} Nights</p>
              </div>
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/10">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-white/50">{roomFloor}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-white">{formatDate(guest.checkIn)}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-amber-400/70" />
                      <div>
                        ...
                        <p className="text-sm text-white">{formatDate(guest.checkOut)}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Preferences */}
                  {(guest.preferences.dietary.length > 0 || guest.preferences.room.length > 0) && (
                    <div className="mt-4 pt-4 border-t border-white/10">
                      {guest.preferences.dietary.length > 0 && (
                        <p className="text-xs text-white/50 mb-1">
                          Dietary: <span className="text-white/70">{guest.preferences.dietary.join(', ')}</span>
                        </p>
                      )}
                      {guest.preferences.room.length > 0 && (
                        <p className="text-xs text-white/50">
                          Room: <span className="text-white/70">{guest.preferences.room.join(', ')}</span>
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
                    {guest.pets && (
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm text-white/80">Traveling with {guest.pets.name} ({guest.pets.type})</p>
                      </div>
                    )}
                    {guest.specialRequests.map((req, idx) => (
                      <div key={idx} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm text-white/80">{req.type}{req.time ? ` at ${req.time}` : ''}</p>
                        </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                    <p className="text-xs text-white/40">
                      Confirmation: <span className="font-mono text-white/70">{guest.confirmationCode}</span>
                    </p>
                    <p className="text-xs text-white/40">{guest.nights} nights • {guest.guestCount} guest{guest.guestCount > 1 ? 's' : ''}</p>
                  </div>
                </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col max-w-md">
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl flex-1 overflow-hidden">
              <VoiceSessionChat
                agentId="concierge"
                sessionId={`guest-${slug}`}
                elevenLabsAgentId={process.env.NEXT_PUBLIC_ELEVENLABS_GUEST_AGENT_ID}
                title="Yuki"
                avatar="/avatars/guest-avatar.jpg"
                welcomeMessage={`Hello ${firstName}! Welcome to The 1898. I'm Yuki, your personal concierge. It's wonderful to have you with us — how can I make your stay special today?`}
                suggestions={[
